<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0" 
	...
	<xsl:variable name="relatedTopicsSection" select="boolean(string-length(/topic/topic/ddue:developerHowToDocument/ddue:relatedTopics[normalize-space(.)]) > 0)"/>
	<xsl:variable name="examplesSection" select="boolean(string-length(/topic/topic/ddue:developerHowToDocument/ddue:codeExample[normalize-space(.)]) > 0)"/>
		...
		<html xmlns:MSHelp="http://msdn.microsoft.com/mshelp" DIR="LTR">
			<head>
				...
				<xml>
                    ...
                    <xsl:choose>
                        <xsl:when test="string-length(topic/ddue:developerHowToDocument/ddue:summary[normalize-space(.)]) > 0">
                            <xsl:call-template name="AbstractTitles">
								<xsl:with-param name="text1" select="topic/ddue:developerHowToDocument/ddue:summary[string(.)]"/>
							</xsl:call-template>
                        </xsl:when>
						<xsl:when test="string-length(topic/ddue:developerHowToDocument/ddue:introduction/p[normalize-space(.)]) > 0">
							<xsl:call-template name="AbstractTitles">
								<xsl:with-param name="text1" select="topic/ddue:developerHowToDocument/ddue:introduction/p[string(.)]"/>
							</xsl:call-template>
                        </xsl:when>
					</xsl:choose>
				</xml>
							...
							<td>
								<xsl:choose>
									<xsl:when test="$relatedTopicsSection">
										<a href="#seeAlsoNoToggle" onclick="OpenSection(seeAlsoNoToggle)"><xsl:text/><includeSSC item="SeeAlso"/><xsl:text/></a>&#160;
									</xsl:when>
								</xsl:choose>
								<xsl:choose>
									<xsl:when test="$examplesSection">
										<a href="#codeExampleSection"><xsl:text/><includeSSC item="Example"/><xsl:text/></a>
									</xsl:when>
								</xsl:choose>
							</td>
						</tr>
	...
	<xsl:template match="ddue:developerHowToDocument">
		<xsl:for-each select="*">
			<xsl:choose>
...
-->
	...
	<xsl:template match="ddue:buildInstructions">
		<h1 class="heading">
			<includeSSC item="buildInstructions"/>
		</h1>
		<div id="buildInstructionsSection" class="section">
			<xsl:apply-templates/>
		</div>
	</xsl:template>
	...
	<xsl:template match="ddue:robustProgramming">
		<h1 class="heading">
			<includeSSC item="robustProgramming"/>
		</h1>
		<div id="robustProgrammingSection" class="section">
			<xsl:apply-templates/>
		</div>
	</xsl:template>
	...
	<xsl:template match="ddue:security">
		<h1 class="heading">
			<includeSSC item="securitySection"/>
		</h1>
		<div id="securitySection" class="seeAlsoNoToggleSection">
			<xsl:apply-templates/>
		</div>
	</xsl:template>
